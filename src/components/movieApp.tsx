...
export class MovieApp extends Component<{}, MovieAppState> {
    ...
    render() {
        ...
        const noMoviesFound = !isLoading && !errorMessage && movieResults.length === 0;

        return (
            <div className="search-results">
                <SearchBar value={searchRequest} onChange={this.updateSearchRequest} onSubmit={this.searchSubmit} />
                {(() => {
                    if (isLoading) return <p className="basic-text">Loading movies...</p>;
                    if (errorMessage) return <ErrorMessage message={errorMessage} />;
                    if (noMoviesFound) return <p className="basic-text">No movies found</p>;
